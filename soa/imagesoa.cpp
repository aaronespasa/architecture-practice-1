...
void ImageSOA::ReadBitmapHeader(BmpHeaderContainer header) {
...
void ImageSOA::WriteBitmapHeader(std::string filename) {
...
int ImageSOA::ReadBitmapFile(std::string filename) {
    ...
    ReadBitmapHeader(header);
    checkBMPHeader(bmpHeader);

    extraHeaderInformation.resize(bmpHeader.offset_data - sizeof(header));
